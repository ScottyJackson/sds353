...
### (d)
The code below demonstrates how the coefficients shrink to 0 as $\lambda$ gets big. This also explains why ridge regression is called a ***shrinkage estimator***,
as it is simultaneously fitting weights under the constraint that the weights are shrunk by some amount controlled by the hyper-parameter $\lambda$.
```{r q8.8.2.d}
Z <- runif(2000, -1, 1)
...
grid <- seq(0, 0.2, by=0.0005)
Beta <- matrix(nrow=50, ncol=length(grid))

for (i in 1:50) {
  for (j in 1:length(grid)) {
    Beta[i,j] <- ridge.coef(X.train[i,], y.train, grid[j])
  }
}
...
cat('Min Lambda: ', ridge.mod$lambda.min, '\n')
